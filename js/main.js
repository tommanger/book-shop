    ...
    var strHTMLs = books.map(function (book) {
        return `<tr>
                        <th scope="row">${book.id}</th>
                            ...
                            <button data-trans="buttonDeleteTrans" data-toggle="modal" data-target="#deleteModal" onclick="renderDeleteModal('${book.id}')" type="button" class="btn btn-success bg-danger">Delete</button>
                        </td>
...
function renderDeleteModal(bookId) {
    var btn = document.querySelector('.del-book-btn');
    btn.dataset.bookid = bookId;

    var books = getBooks()
    var book = getItemByID(bookId, books);
    var modalBody = document.querySelector('.modal-del-body');
    modalBody.innerHTML = `
    <span data-trans="nameTrans">Name: </span> ${book.name} <br>
    <span data-trans="idTrans">Id </span>: ${bookId}`
    doTrans();
